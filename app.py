...
# --- פונקציית עזר לטעינת קובץ CSS ---
def load_css(file_name):
    try:
        with open(file_name, encoding='utf-8') as f:
            st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)
    except FileNotFoundError:
        st.error(f"שגיאה: קובץ CSS בשם '{file_name}' לא נמצא. אנא ודא שהוא באותה תיקייה כמו app.py.")
    except Exception as e:
        st.error(f"שגיאה בטעינת קובץ CSS: {e}")

# טען את קובץ ה-CSS בתחילת האפליקציה
load_css('style.css')
...
def display_member_card(member: FamilyMember):
    """מציג פרטים של בן משפמה בודד בפורמט כרטיס."""
...
def add_edit_member_form():
    ...
    with st.form("member_details_form"):
        ...
        # Main form submit button for member details (moved to end of main form)
        ...
        with add_gift_col:
            ...
            if st.button("➕ הוסף רעיון", key=f"add_gift_btn_outside_form_{editing_id or 'new'}"):
                if new_gift_idea_input and new_gift_idea_input.strip() and new_gift_idea_input.strip() not in current_gift_ideas:
                    ...
                    st.session_state[f"new_gift_idea_input_outside_form_{editing_id or 'new'}"] = ""
                    st.rerun()
        ...
        for i, entry_dict in enumerate(current_gift_history_dicts):
            ...
            with st.expander(f"מתנה ב-{entry_obj.date_given.strftime('%d/%m/%Y')} - {entry_obj.description}"):
                with st.form(key=f"edit_gift_history_form_{editing_id}_{i}_standalone"):
                    ...
                    with col_update:
                        if st.form_submit_button("עדכן מתנה זו", key=f"update_gift_history_button_{editing_id}_{i}"):
                            try:
                    ...
                    with col_delete:
                        if st.form_submit_button("🗑️ מחק מתנה זו", key=f"delete_gift_history_button_{editing_id}_{i}_delete"):
...
def calendar_page():
    ...
    # הסר את תגית <style> מכאן, היא נטענת באמצעות load_css בתחילת הקובץ
    ...
    if requested_page in ["home", "add_member", "list_all", "settings", "edit_member", "calendar"]:
        ...
        (requested_page == "edit_member" and st.session_state.get("editing_member_id") is None):
            ...
            if requested_page == "add_member":
                ...
                pass
            else: # home, list_all, settings, calendar
                st.session_state.editing_member_id = None
                clear_form_session_state()
